...
class Chef
  class Resource
    # A resource used to add and remove non-XA data-sources to the JBoss
    # instance.
    #
    # @provides jboss_datasource
    #
    # @action add
    ...
    #
    class JBossDatasource < Chef::Resource
      include JBoss
  ...
  class Provider
    class JBossDatasource < Chef::Provider
      include JBoss
      ...
      def initialize(new_resource, run_context)
        super
        @path = "/subsystem=datasources/data-source=#{r.name}"
      end
      ...
      def load_current_resource
        @current_resource ||= Chef::Resource::JBossDatasource.new(r.name)
